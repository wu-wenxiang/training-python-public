...
def getAlarmData(csv, bsData):
    bsDict =  bsData.to_dict('index')
    alarmData = pd.read_csv(csv, header=None, dtype=str)
    alarmData.columns = ['告警流水号', '网管告警ID', '告警发生时间', '告警恢复时间', '告警对象名称']
    alarmData = alarmData[[i in bsDict for i in alarmData['告警对象名称']]]
    alarmData['告警对象ID'] = alarmData['告警对象名称'].apply(lambda x:bsDict[x]['基站ID'])
    alarmData['工程状态'] = alarmData['告警对象名称'].apply(lambda x:bsDict[x]['基站状态'])
    alarmData['省'] = alarmData['告警对象名称'].apply(lambda x:bsDict[x]['省区域ID'])
    alarmData['地市'] = alarmData['告警对象名称'].apply(lambda x:bsDict[x]['市区域ID'])
    alarmData['区县'] = alarmData['告警对象名称'].apply(lambda x:bsDict[x]['所属区县ID'])
